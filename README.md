Muro update utilities
=========================

This is a collection of firmware upgrade tools specifically written
for Muro based on FreeBSD ideas (kernel and base sets) and tools
...
usable from the command line. Since Muro uses FreeBSD's package
...
consistency of all the Muro files.  It can also wipe the
...
root certificates -> HTTPS -> Muro package fingerprints.

What it will also do is turn a supported stock FreeBSD release into
an Muro installation.  Both UFS and ZFS installations are supported.
...
After successful reboot, Muro should be up and running.  :)
...
revert any package to a previous state of a particular Muro
release.
